...
class AmazonDataset(object):
    ...
    def load_reviews(self):
        ...
        word_count = 0
        line_index = 0
        _, time_clusters = load_timeClusters(DIR_DATASET[self.data_dir.strip('/')])
        for line in self._load_file(self.review_file):
            arr = line.split('\t')
            user_idx = int(arr[0])
            product_idx = int(arr[1])
            word_indices = [int(i) for i in arr[2].split(' ')]  # list of word idx
            review_data.append((user_idx, product_idx, word_indices, time_clusters[line_index])) 
...
class AmazonDataLoader(object):
    ...
    def get_batch(self):
        ...
        user_idx, product_idx, text_list, cluster_num = self.dataset.review.data[review_idx]
        ...
        while len(batch) < self.batch_size:
            ...
            if random.random() < self.dataset.word_sampling_rate[word_idx]:
                data = [user_idx, product_idx, word_idx, cluster_num] # data[:3:8]
