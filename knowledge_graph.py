...
class KnowledgeGraph(object):
    ...
    def _load_reviews(self, dataset, word_tfidf_threshold=0.1, word_freq_threshold=5000):
        ...
        for rid, data in enumerate(dataset.review.data):
            uid, pid, review, clustersNum = data
            ...
            # (2) Add edges.
            self._add_edge(USER, uid, PURCHASE[clustersNum], PRODUCT, pid)
            num_edges += 2
            for wid in remained_words:
                self._add_edge(USER, uid, MENTION[clustersNum], WORD, wid)
                self._add_edge(PRODUCT, pid, DESCRIBED_AS[clustersNum], WORD, wid)
                num_edges += 4
